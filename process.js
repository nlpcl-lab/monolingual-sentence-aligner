...
const raw_data = JSON.parse(fs.readFileSync(IN_FILE, 'utf-8')).map((obj) => new EditPair(obj));
...
fs.writeFileSync(OUT_FILE, alignments.map((align) => align.toExportString()).join('\n'), 'utf-8');

console.log("Computiung stats:");
// Stats
let one2one = 0; // single sentence - single sentence
let one2many = 0, many2one = 0; // single sentence - multiple sentences
let many2many = 0; // multiple sentences rearranged
let deletions = 0, additions = 0; // sentence deletions and additions
...
console.log("%d sentences paired. 1:m %d / n:1 %d / n:m %d", one2one, one2many, many2one, many2many);
console.log("%d sentences from orig and %d sentences from rev are not paired.", deletions, additions);
