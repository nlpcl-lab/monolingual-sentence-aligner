...
const TOKEN = {'STR': '=', 'DEL': '-', 'INS': '+', 'SBD': '/', 'DEL_BD': '-/', 'INS_BD': '+/'};
TOKEN.isBorder = (x) => x === TOKEN.SBD || x === TOKEN.DEL_BD || x === TOKEN.INS_BD;
...
class Diff {
	constructor(str){
		...
		while((result = regex.exec(str)) !== null){
			switch(result[1]){
				case 'ins':
					if(result[2]) tokens.push({'type': TOKEN.INS, 'value': unescapeHTML(result[2])});
					break;
				case 'del':
					if(result[2]) tokens.push({'type': TOKEN.DEL, 'value': unescapeHTML(result[2])});
					break;
				default:
	...
	addSBs(){
		...
		this.tokens.forEach((token) => {
			...
			let need_cut = (stick, ruler) => ruler != null && stick.length >= ruler.length;
			let add_token = (type, value) => value && new_tokens.push({'type': type, 'value': value});
			let cut_tokens = (token_type, added_string) => {
				...
				while(need_cut(curr_buffer, curr_target)){
					...
					add_token(token_type, curr_sliced === 0 ? curr_buffer.slice(curr_buffer_init_len, curr_target.length) : curr_target);
			...
			switch(token.type){
				...
				case TOKEN.STR:
					...
					let base_orig = 0, base_rev = 0, base_all = 0;

					if(orig === ""){
						orig = token_trimmed; base_orig = token_trimmed_len;
					} else orig += token.value;
					if(rev === ""){
						rev = token_trimmed; base_rev = token_trimmed_len;
					} else rev += token.value;

					const DONT_CUT = token.value.length + 1;
					while((need_cut_orig = need_cut(orig, curr_orig_sent)) | (need_cut_rev = need_cut(rev, curr_rev_sent))){
						let cut_orig_ind = DONT_CUT, cut_rev_ind = DONT_CUT, cut_min_ind;
						...
						if(need_cut_orig){
							...
							if(cut_orig_ind === 0) cut_orig_ind = curr_orig_sent.length - curr_orig_len;
							else cut_orig_ind += curr_orig_sent.length;
						}
						if(need_cut_rev){
							cut_rev_ind = base_rev;
							if(cut_rev_ind === 0) cut_rev_ind = curr_rev_sent.length - curr_rev_len;
						...
						if(cut_orig_ind === cut_min_ind){
							...
							while(cut_orig_ind < token.value.length && token.value[cut_orig_ind].match(/\s/)) cut_orig_ind++;
						...
						if(cut_rev_ind === cut_min_ind){
							...
							while(cut_rev_ind < token.value.length && token.value[cut_rev_ind].match(/\s/)) cut_rev_ind++;
		...
		while(TOKEN.isBorder(new_tokens[new_tokens.length-1].type)) new_tokens.pop();
	...
	removeWhitespaces() {
		let new_tokens = [];
		let after_del_border = true, after_ins_border = true;
		// Remove whitespaces
		this.tokens.forEach((token) => {
			let do_trim = false;
			switch(token.type){
			...
			if(TOKEN.isBorder(token.type) || !!token.value){
				if(!TOKEN.isBorder(token.type)) after_del_border = after_ins_border = false;
	...
	reorderTokens() {
		...
		this.tokens.forEach((token) => {
			switch(token.type){
				case TOKEN.DEL: case TOKEN.DEL_BD:
					del_list.push(token); break;
				case TOKEN.INS: case TOKEN.INS_BD:
					ins_list.push(token); break;
				default:
					add_tokens(); new_tokens.push(token);
			}
		});
		add_tokens();
	}
	...
	getOriginal(){
		return this.tokens.filter(
			(token) => token.type === TOKEN.STR || token.type === TOKEN.DEL
		).map((token) => token.value).join('');
	}
	getRevised(){
		return this.tokens.filter(
			(token) => token.type === TOKEN.STR || token.type === TOKEN.INS
		).map((token) => token.value).join('');
	}
	toString(){
...
class EditPair {
	constructor(obj) {
		// metadata
		this.id = obj.id; this.tags = obj.tags;
	...
	checkDiff() {
		if(this.diff.getOriginal().replace(/\s+/g, ' ').trim() !== unescapeHTML(obj.original).replace(/\s+/g, ' ').trim()){
			console.log("%s: original is different", this.id);
		}
		if(this.diff.getRevised().replace(/\s+/g, ' ').trim() !== unescapeHTML(obj.revision).replace(/\s+/g, ' ').trim()){
