...
let inclusion_dist = (x, y) => levenshtein.get(x, y) - Math.abs(x.length - y.length);
...
class DisjointSet {
	...
	union(that) {
		let xroot = this.getRoot(), yroot = that.getRoot();
...
class Alignment {
	...
	toString() {
		return `[Alignment ${this.xl}x${this.yl} for ${this.editPair.id}]\n${this.getClusterString()}`;
	}
	toExportString() {
	...
	getClusterString() {
		return this.getClusters().map(([xs, ys]) => {
			let xStr = xs.map((ind) => `\t${this.xSents[ind].replace(/\s/g, ' ')}`).join('\n');
			let yStr = ys.map((ind) => `\t${this.ySents[ind].replace(/\s/g, ' ')}`).join('\n');
...
Alignment.useDiff = (editPair) => {
	...
	editPair.diff.tokens.forEach((token) => {
		if(TOKEN.isBorder(token.type)){
			...
			if(token.type === TOKEN.STR && token.value.trim()) align.addEdge(currX, currY);
		}
	});
	let xOrphans = new Set(), yOrphans = new Set();
	let xNSents = [], yNSents = []; // Normalized sentences
	...
	for(let xo of xOrphans) yLoop: for(let yo of yOrphans){
	...
	let processOrphans = (self, other, addEdge) => {
		for(let o of self.orphans){
			...
			while(o_after < self.edges.length-1 && self.orphans.has(o_after)) o_after++;
			...
			let min_cost = COST_DISCONNECTED + sent.length, min_c = -1;
	...
	let iter = 0;
	let xObj = {'orphans': xOrphans, 'nSents': xNSents, 'sents': align.xSents, 'edges': align.xEdges};
	let yObj = {'orphans': yOrphans, 'nSents': yNSents, 'sents': align.ySents, 'edges': align.yEdges};
